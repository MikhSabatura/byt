...
public class Bank {
    ...
    public void openAccount(String accountid) throws AccountExistsException {
        if (accountlist.containsKey(accountid)) {
            ...
            accountlist.put(accountid, new Account(accountid, currency)); //this line contained a bug - it got account instead of creating it
            //it caused exception in AccountTest setUp method
        }
    }
    ...
    public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
        if (!accountlist.containsKey(accountid)) { //this line caused a bug: it didn't have a negation of the condition
    ...
    public void addTimedPayment(String accountid, String payid, Integer interval, Integer next, Money amount, Bank tobank, String toaccount) throws AccountDoesNotExistException {
        /*this method caused null pointer exception as it didn't handle the case when payment was assigned to a non-existent account
          and caused null pointer exception in testTimedPayment()*/
        Account account = accountlist.get(accountid);
        if (account == null) {
            throw new AccountDoesNotExistException();
        }
